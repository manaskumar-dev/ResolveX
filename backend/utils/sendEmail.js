...
const sendEmail = async (username, email, ticketId, status) => {
    try {
        const response = await axios.post(
            'https://api.brevo.com/v3/smtp/email',
            {
                ...
                htmlContent: `<p>Hi ${username},</p><p>Your complaint with Ticket ID: <strong>${ticketId}</strong> ${status}</p><p>Thank you.</p>`,
            },
            {
                headers: {
